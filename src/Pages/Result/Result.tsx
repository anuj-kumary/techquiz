...
export const Result = () => {
    ...
    const points = results.reduce((acc, curr) => {
        ...
        return options[curr.selectedOption]?.isRight ? acc + eachQuestionPoint : acc;
    }, 0);
    ...
    return (
        <main className='main'>
            ...
            <p className='text text__center'>Final Score: {points}/{eachQuestionPoint * totalQuestions}</p>
                                ...
                                {el.options.map((option, index) => {
                                    return (
                                        <div key={index}>
                                            {
                                                rightQuestion ? (
                                                    <div className={`right__answer text ${option.isRight ?
                                                        'background__right'
                                                        : 'background__normal'
                                                        }`}>
                                                        <p>{option.value}</p>
                                                    ...
                                                    <div className={`right__answer text ${el.selectedOption === index ? 'background__wrong'
                                                        : option.isRight
                                                            ? 'background__right'
                                                            : 'background__normal'
                                                        }`}>
