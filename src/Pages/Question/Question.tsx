...
export const Question = () => {
  ...
  }, [dispatch, questionIndex, state.answers.length]);
  ...
  const dispatchQuizAnswer = (
    ...
    if (questionIndex === questions?.length) navigate(`/${id}/result`);
    else navigate(`/${id}/${Number(questionIndex) + 1}`, { replace: true });
    setActiveButton(-1);
    setTimer(60);
  };
    ...
    <div className='quiz__container'>
      <div className='box'>
        <header className='quiz__header'>
          <span className='text float__left'>
            Question: {questionIndex} / {questions?.length}
          </span>
          <span className='timer text float__right'>
            ...
            return (
              <li
                ...
                className={`text answer ${
                  index === Number(activeButton) ? 'quiz__selected_answer' : ''
                }`}
              >
                {el.value}
              </li>
